...
* 函数: initGlobalPara
* 功能: 初始化全局变量;
* g_MonConfigNumbToStr 存储treeview, grid 属性指标数字字符串映射表, 将其挂载到window对象上。
* g_treeView: TreeView的引用对象，为了方便其他函数使用，将其设置为当前页面全局对象。
*/
function initGlobalPara () {
  g_treeView = null;
  window.g_MonConfigNumbToStr = [];
  window.g_MonConfigName = ["ObjectIDNS", "DomainNS", "AttrName"];
  window.g_monConfigObjectIDName = "ObjectIDNS";
  window.g_monConfigDomainName = "DomainNS";
  window.g_monConfigAttrName = "AttrName";
  for (var i = 0; i < g_MonConfigName.length; ++i) {
    g_MonConfigNumbToStr[g_MonConfigName[i]] = [];
  }
  // g_MonConfigNumbToStr["ObjectIDNS"] = [];
  // g_MonConfigNumbToStr["DomainNS"] = [];
  // g_MonConfigNumbToStr["AttrName"] = [];
  // console.log(g_MonConfigNumbToStr)
  window.g_MonConfigStrToNumb = g_MonConfigNumbToStr;
}
...
function initMonConfigMapData () {
  var monConfigInfoFieldArray = new Array(g_MonConfigName.length);
  var monConfigRspData = [];
  var isReqMonConfigEnd = [];

  for (var index = 0; index < g_MonConfigName.length; ++index) {
    monConfigInfoFieldArray[index] = new userApiStruct.CShfeFtdcReqQryMonConfigInfoField();
    monConfigInfoFieldArray[index].ConfigName = g_MonConfigName[index];
    monConfigRspData[g_MonConfigName[index]] = "";
    isReqMonConfigEnd[g_MonConfigName[index]] = false;
  }
      ...
      if (undefined !== monConfigRspData[pRspQryMonConfigInfo.ConfigName]) {
        ...
        if (isAllRspEnd) {
          ...
          for (var ConfigName in isReqMonConfigEnd) {
            ...
            g_MonConfigNumbToStr[ConfigName] = processMonConfigInfoData(monConfigRspData[ConfigName]);
          }
...
function reqTreeViewData () {
  ...
  userApi.emitter.on(monitor2ObjectField.rspMessage, function(data) {
    ...
    // console.log (g_monConfigObjectIDName);
    pRspQryMonitor2Object.ObjectID = g_MonConfigNumbToStr[g_monConfigObjectIDName][pRspQryMonitor2Object.ObjectID];
...
function setMonConfigStrToNumbData () {
  ...
  // console.log (g_MonConfigStrToNumb)
}
...
function onSelectGridData_oldApi (e) {
  var dataItem, reqQryMonOidRelationData, reqQryMonOidRelationField, rspData, uri;
  dataItem = g_treeView.dataItem(e.node);
  reqQryMonOidRelationData = new userApiStruct.CShfeFtdcReqQryMonOidRelationField();
  reqQryMonOidRelationData.ObjectID = dataItem.id;
  reqQryMonOidRelationField = {};
  reqQryMonOidRelationField.reqObject = reqQryMonOidRelationData;
  reqQryMonOidRelationField.RequestId = ++window.ReqQryMonOidRelationTopicRequestID;
  reqQryMonOidRelationField.rspMessage = EVENTS.RspQryMonOidRelationTopic + reqQryMonOidRelationField.RequestId;
  window.reqQryMonOidRelationField = reqQryMonOidRelationField;
  window.isPageID = true;
  rspData = [];
  
  userApi.emitter.emit(EVENTS.ReqQryMonOidRelationTopic, reqQryMonOidRelationField);
  userApi.emitter.on(reqQryMonOidRelationField.rspMessage, function(data) {
    var gridDataEventName, gridID;
    if (data.hasOwnProperty('pRspQryMonOidRelation')) {
      rspData.push(data.pRspQryMonOidRelation);
      if (data.bIsLast === true) {
        gridID = getObjectID(data.pRspQryMonOidRelation.ObjectID);
...
function onSelectGridData (e) {
  ...
  reqQryMonOidRelationData = new userApiStruct.CShfeFtdcReqQryMonOidRelationField ();
  reqQryMonOidRelationData.ObjectID =  g_MonConfigStrToNumb[g_monConfigObjectIDName][dataItem.id];
  reqQryMonOidRelationField = {};
  reqQryMonOidRelationField.reqObject = reqQryMonOidRelationData;
  reqQryMonOidRelationField.RequestId = ++window.ReqQryMonOidRelationTopicRequestID;
  reqQryMonOidRelationField.rspMessage = EVENTS.RspQryMonOidRelationTopic + reqQryMonOidRelationField.RequestId;
  ...
  window.isPageID = true;
  rspData = [];
  // console.log (reqQryMonOidRelationField);
  userApi.emitter.emit(EVENTS.ReqQryMonOidRelationTopic, reqQryMonOidRelationField);
  userApi.emitter.on(reqQryMonOidRelationField.rspMessage, function(data) {
    var gridDataEventName, gridID;
    if (data.hasOwnProperty('pRspQryMonOidRelation')) {
      var transData = data.pRspQryMonOidRelation
      // transData.HoldAttrID = g_MonConfigNumbToStr[g_monConfigDomainName][data.pRspQryMonOidRelation.HoldAttrID]
      transData.ObjectID = g_MonConfigNumbToStr[g_monConfigObjectIDName][data.pRspQryMonOidRelation.ObjectID]
      rspData.push(transData);
      if (data.bIsLast === true) {
        // console.log (data.pRspQryMonOidRelation.ObjectID);
        // gridID =  transData.ObjectID;
        gridID = getObjectID(transData.ObjectID);
        gridDataEventName = gridID;
        // console.log (gridID)
        console.log (rspData)
        userApi.emitter.emit(gridDataEventName, {
