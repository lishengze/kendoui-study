...
function setup(gridViewPointer) {
  ...
  initGlobalData(gridViewPointer);

  gridViewPointer = initGridViewPointer(gridViewPointer);
...
function initGlobalData(gridViewPointer) {
  window.configData = getConfigData();
  g_GlobalChart[gridViewPointer.pageID] = [];
  g_gridToolbarData = {title : '属性列表' }
  g_borderWidth = 6
  g_screenLeft = 587;
  g_toolbarHeight = 20.5
  g_toolbarModel = kendo.template("<strong style = 'color:indianred'>#: title #  </strong>\
                                    <i  class = 'gridMax fa fa-clone'></i>\
                                    <i  class = 'gridClose fa fa-times'></i>")
}

function initGridViewPointer(gridViewPointer) {
...
function initializeGrid(gridViewPointer) { // 初始化 grid 属性列表 表格
  ...
  gridSelector.kendoGrid({
    scrollable: true,
    resizable: true,
    toolbar:  g_toolbarModel(g_gridToolbarData),
    columns: [
      ...
      if( gridViewPointer.times % 2 === 1) {
        ...
        if(isItemHasBeenChoosen === false) {  // 如果该对象注册过，那么不再执行后面的注册函数,避免一个属性注册多次双击事件
          ...
          selectedRows.dblclick(function(e) {
            ...
            var HoldAttrID = selectedText;

            console.log ('ObjectID: ' + objectID);
            console.log ('HoldAttrID: ' + HoldAttrID);

            if (gridViewPointer.ChartItem[HoldAttrID] !== true) {
              initializeChart(gridViewPointer, HoldAttrID);
              reqQrySubscriberFunc(objectID, HoldAttrID);
              gridViewPointer.ChartItem[HoldAttrID] = true;
            }
...
function initializeChart(gridViewPointer,chartID) {
  ...
  curChartToolbarSelector.html(g_toolbarModel(highchartsToolbar));
...
function setChartDataGlobal(gridViewPointer, chartID, curChart){
  ...
  console.log('rtnDataName: ' + rtnDataName)
  ...
  userApi.emitter.on (rtnDataName, function(data){
    ...
    var val = Math.round(parseFloat(data.AttrValue)*100/100)
    // var curRtnData = [tranTimeToUTC(data.MonDate, data.MonTime), val];
    var curRtnData = [data.MonTime * 1000, val];
...
function transformGridData(originalData) { // 根据XML文件映射关系,通过指标ID 获取指标名称
    ...
    for (var tmpindex = 0; tmpindex < originalData.length; ++tmpindex) {
      // console.log(originalData[tmpindex].HoldAttrID);
      tmpItem = {
        'itemName': g_MonConfigNumbToStr[g_monConfigAttrName][originalData[tmpindex].HoldAttrID],
        'itemId' :  g_MonConfigNumbToStr[g_monConfigDomainName][originalData[tmpindex].HoldAttrID],
        'itemValue': 0,
        'time': 0,
      }
...
*/
function registerRtnObjectAttrTopic_OldServer() {
...
function registerRtnObjectAttrTopic() {
  if (window.registerRtnObjectAttrTopic === false) {
    userApi.emitter.on(EVENTS.RtnMonObjectAttr, function(data){
      var curRtnMessageName = g_MonConfigNumbToStr[g_monConfigObjectIDName][data.ObjectID] + '.' 
                            + g_MonConfigNumbToStr[g_monConfigDomainName][data.AttrType];
      // console.log (data)
      userApi.emitter.emit(curRtnMessageName, data);
    });
    window.registerRtnObjectAttrTopic = true;
...
function reqQrySubscriberFunc_OldServer(objectID, attrType){ // 注册订阅信息
...
function reqQrySubscriberFunc(objectID, HoldAttrID){ // 注册订阅信息
  var reqSubscribeData = new userApiStruct.CShfeFtdcReqSubscribeField();
  reqSubscribeData.ObjectID = g_MonConfigStrToNumb[g_monConfigObjectIDName][objectID];
  reqSubscribeData.AttrType = g_MonConfigStrToNumb[g_monConfigDomainName][HoldAttrID];
  var reqSubscribeField = {}
  reqSubscribeField.reqObject  = reqSubscribeData;
  reqSubscribeField.RequestId  = ++window.ReqSubscribeRequestID;
  reqSubscribeField.rtnMessage = objectID + '.' + HoldAttrID;

  // registerRtnObjectAttrObjectID(reqQrySubscriberField.rtnMessage);
  // console.log (reqSubscribeField)
  userApi.emitter.emit(EVENTS.ReqSubscribe, reqSubscribeField);
}
  ...
  if(gridViewPointer.screenSelect < 4 && gridViewPointer.screenSelect > 0) {
    for(var th = 0; th < gridViewPointer.screenSelect; th ++) {
      ...
      if(true === $(node).hasClass('UsageModel')) {
        $(node).css({'top' : gridViewPointer.containerHeight* th / gridViewPointer.screenSelect + 5, 'left' : gridViewPointer.containerLeft, 'width' : gridViewPointer.screenWidth + g_borderWidth, 'height' : gridViewPointer.containerHeight/ gridViewPointer.screenSelect + g_borderWidth - 15})
        var chart = $(node).find('.highstockChart').highcharts()
        chart.setSize($(node).width() , $(node).height() - g_toolbarHeight, false );
      } else {
   ...
   if(true === $(node).hasClass('UsageModel')) {
     $(node).css({'top' : 0 + 5, 'left' : gridViewPointer.containerLeft, 'width' : gridViewPointer.screenWidth / 2 + g_borderWidth - 15, 'height' : gridViewPointer.containerHeight / 2 + g_borderWidth - 15})
     var chart = $(node).find('.highstockChart').highcharts()
     chart.setSize($(node).width(), $(node).height() - g_toolbarHeight  , false);
   } else {
   ...
   if(true === $(node).hasClass('UsageModel')) {
     $(node).css({'top' : 0 + 5, 'left' : gridViewPointer.containerLeft + gridViewPointer.screenWidth / 2 - 5, 'width' : gridViewPointer.screenWidth / 2 + g_borderWidth - 15, 'height' : gridViewPointer.containerHeight / 2 + g_borderWidth - 15})
     var chart = $(node).find('.highstockChart').highcharts()
     chart.setSize($(node).width(), $(node).height() - g_toolbarHeight  , false);
   } else {
   ...
   if(true === $(node).hasClass('UsageModel')) {
     $(node).css({'top' : gridViewPointer.containerHeight / 2 + 5, 'left' : gridViewPointer.containerLeft, 'width' : gridViewPointer.screenWidth / 2 + g_borderWidth - 15, 'height' : gridViewPointer.containerHeight / 2 + g_borderWidth - 15})
     var chart = $(node).find('.highstockChart').highcharts()
     chart.setSize($(node).width(), $(node).height() - g_toolbarHeight  , false );
   } else {
   ...
   if(true === $(node).hasClass('UsageModel')) {
     $(node).css({'top' : gridViewPointer.containerHeight / 2 + 5, 'left' : gridViewPointer.containerLeft + gridViewPointer.screenWidth / 2 - 5, 'width' : gridViewPointer.screenWidth / 2 + g_borderWidth - 15, 'height' : gridViewPointer.containerHeight / 2 + g_borderWidth - 15})
     var chart = $(node).find('.highstockChart').highcharts()
     chart.setSize($(node).width(), $(node).height() - g_toolbarHeight  , false );
   } else {
...
function resizeNode(gridViewPointer,node) {
  // var g_toolbarHeight = 2 * parseInt($('#CPUUsageModel' + index ).css('border-width')) + $('.k-grid-toolbar').height() //计算出边框和toolbar的高度。便于设定 Highcharts高度
  ...
  if (nodeMaxWidth !==  $(node).width()) { //最大化
    ...
    if (true === $(node[0]).hasClass('UsageModel')) {
      ...
      chart.setSize(nodeMaxWidth , nodeMaxHeight - g_toolbarHeight, false );
    }
  } else {// 还原
     ...
     if (true === $(node[0]).hasClass('UsageModel')) {
       var chart = $(node[0]).find('.highstockChart').highcharts()
       chart.setSize(nodeWidth, nodeHeight - g_toolbarHeight , false );
     }
  }
}
