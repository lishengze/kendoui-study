...
function setup(index) {
  ...
  // var toolbarHeight = 15 + 2 *  parseInt($('#CPUUsageModel' + index ).css('border-width')) //计算出边框和toolbar的高度。便于设定 Highcharts高度,其中$('.k-grid-toolbar').height = 15
  ...
  $('.SplitScreenBtn').click(function(e) {
    if(this.id === 'BinaryScreen') {
      ...
      if(true === $(node).hasClass('CPUUsageModel')) {
        var chart = $('#CPUUsage' + index).highcharts()
        chart.setSize(screenWidth, containerHeight/ 2 , false );
        $(node).css({'top' : containerHeight/ 2 +10, 'left' : containerLeft, 'width' : screenWidth,'height' : containerHeight/ 2 - 10})
      } else {
      $(node).css({'top' : 0, 'left' : containerLeft, 'width' : screenWidth,'height' : containerHeight/ 2})
      node = nodeQueue.pop()
      }
  ...
  function CPUUsage() {
    ...
    $('#CPUUsageModel' +  index).css({'left' : containerLeft, 'top' : 0, 'width' : screenWidth, 'height' : containerHeight})
    console.log($('#CPUUsageModel' +  index).width())
