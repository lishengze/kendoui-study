...
function initializeGrid() { // 初始化 grid 属性列表 表格
...
function registerGridItemDblClickFunc() { // 生成选中的属性 对应的chart 对象
  var selectedRows = this.select();
  // console.log($(selectedRows[0].parentElement))
  // console.log($(selectedRows[0].parentNode)[0].lastChild.textContent)
    selectedRows.dblclick(function(e) {
      times ++
      if( times % 2 === 1) {
        var selectedNode = null;
        var selectedText = $(selectedRows[0].parentNode)[0].lastChild.textContent;
...
function nodePosition(num) { // 根据分屏要求，确定对象位置
  // var Spacing =
