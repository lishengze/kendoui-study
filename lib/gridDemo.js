...
function setup() {
  ...
  var parentNode = $('.gridOne').parent().parent()
  function onChange() {
    ...
    switch (selectedRows[0].textContent) {
      case '日志事件':
      case 'SyslogEvent':
        $(pNode).find('.rizhi').show()
        console.log($(pNode).find('.rizhi'));
        break
      case '业务进程CPU使用率':
      case 'CPUUsage':
        $(pNode).find('.CPUUsageModel').show()
        break
      default:
   }
  };
  SyslogEvent(parentNode)
  CPUUsage(parentNode)
  // 鼠标响应事件
  var gridWidth  = 0
  var gridHeight = 0
  ...
  function resizeWindow(node) {
    ...
    var toolabrHeigth = 2 *  parseInt($('.CPUUsageModel').css('border-width')) + $('.k-grid-toolbar').height() //计算出边框和toolbar的高度。便于设定 Highcharts高度
    ...
    if(nodeMaxWidth !==  $(node).width()) {
      ...
      if (true === $(node[0]).hasClass('CPUUsageModel')) {
        var chart = $(node[0]).find('.CPUUsage').highcharts()
  ...
  //    resize: function() {
  //        console.log($(this))
  //        var chartNode = $(this).find('.CPUUsage')
  ...
  function SyslogEvent(parentNode) {
    var gridrizhidata = {title : '日志事件' }
    var rizhiNode = $(parentNode).find('.rizhi')
  ...
  function CPUUsage(parentNode) {
    ...
    var CPUNode = $(parentNode).find('.CPUUsage')
    $(CPUNode).highcharts('StockChart', {
      chart: {
        animation: false,
        marginTop: 30
        },
