      ...
      if( gridViewPointer.times % 2 === 1) {
        ...
        for(var i = 0; i < gridViewPointer.itemsArray.length; i ++){
          if(selectedText === gridViewPointer.itemsArray[i]){
            ...
            $(gridViewPointer.chartData.find($('#'+ selectedText + gridViewPointer.gridID + 'Model'))).show()
            console.log('show')
            break
          }
        }
...
var SortDivHandler = {
  ...
  Initialize: function() {
    var isStart = false;
    var isDrag = true;
    $('.baobiaoContainer').delegate('.k-grid-toolbar','mousedown',function(e) {
      console.log($(this))
...
function resizeNode(gridViewPointer,node) {
  ...
  $(node).css('z-index', ++ gridViewPointer.zIndex)
  console.log(gridViewPointer.MaxMinClickedTimes )
