...
var containerLeft = 0 ; //$('#FourSplitScreen').position().left + $('#FourSplitScreen').outerWidth() + 5
var screenLeft = 587;
...
function setup(index) {
  containerLeft = $('#FourSplitScreen' + index).position().left + $('#FourSplitScreen'+ index).outerWidth() + 5;
  screenWidth = $('.baobiaoContainer').width() - containerLeft - 20
  containerHeight = window.innerHeight - 50
  $('#gridOne' + index).outerWidth($('#FourSplitScreen' + index).position().left +  $('#FourSplitScreen' + index).outerWidth() - borderWidth)
  $('#gridOne' + index).height(window.innerHeight -  $('#FourSplitScreen'+ index).outerHeight() - 100)
  $(window).resize(function() {// 根据窗口大小自动调整treeview窗口高度
   //process here
    windowHeight = window.innerHeight
    var resizeHeight = windowHeight -  $('#FourSplitScreen' + index).outerHeight() -100
    $('#gridOne' + index).outerWidth($('#FourSplitScreen' + index).position().left +  $('#FourSplitScreen' + index).outerWidth() - borderWidth)
  ...
  // function onChange() { // kendo Grid自带的 change 事件，但是只能响应单击事件
